...
\prob{}{	\label{3.1c}
	The identity $\vsigT = -\sigw \vsig \sigw$ allows us to rewrite the $\psiL$ transformations in the unitarily equivalent form
	\eq{
		\psi' \to \psi' \paren{ 1 + i \vtht \vdot \frac{\vsig}{2} + \vbet \vdot \frac{\vsig}{2} },
	}
	where $\psi' = \psiLT \sigw$.  Using this law, we can represent the object that transforms as $(\frac{1}{2}, \frac{1}{2})$ as a $2 \times 2$ matrix that has the $\psiR$ transformations law on the left and, simultaneously, the transposed $\psiL$ transformation on the right.  Parametrize this matrix as
	\eq{
		\mqty(
			\Vo + \Ve & \Vq - i \Ve \\
			\Vq + i \Vw & \Vo - \Ve
...
\sol{
	...
	since $\sigm$ forms a complete basis.  This is identical to Eq.~\refeq{thing2c}, so we have shown that $\Vm$ transforms as a 4-vector. \qed
}