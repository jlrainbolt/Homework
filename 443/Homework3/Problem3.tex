...
\sol{
	...
	so
	\eq{
		\ddGam = \frac{\mu^2}{4 M (m^2 + p^2)} \ddcpfw \del^4(M - 2 \sqrt{m^2 + p^2}).
	}
	Integrating this expression over momentum space yields
	\al{
		\Gam &= \frac{\mu^2}{4 M} \int \ddcpfw \frac{1}{m^2 + p^2} \del^4(M - 2 \sqrt{m^2 + p^2}) \\
		&= \frac{\mu^2}{4 \pi M} \int \ddp \frac{p^2}{m^2 + p^2} \del(M - 2 \sqrt{m^2 + p^2}) \\
		&= \frac{\mu^2}{4 \pi M} \int \ddp \frac{4 (m^2 + p^2) - 4 m^2}{4 (m^2 + p^2)} \del[ M - \sqrt{4 (m^2 + p^2)} ] \\
		&= \frac{\mu^2}{4 \pi M} \frac{M^2 - 4 m^2}{M^2},
	}
	so the lifetime is
	\eq{
		\tau = \frac{1}{\Gam}
		= \ans{ \frac{4 \pi}{\mu^2} \frac{M^3}{M^2 - 4 m^2}. }
	}
	\qed
}