...
\sol{
	For \ref{1b}, we use the Mathematica notebook from Ref.~\cite{Hartle}:
	
	\scalebox{0.625}{
		\includegraphics{1c-b}
	}
	
	 Here $r \to 1$, $\tht \to 2$, and $\phi \to 3$.  Taking into account that in a coordinate basis $\Gamsabg$ is symmetric in its last two indices~\cite[p.~1172]{MCP}, these match our result from \ref{1b}.
	
	For \ref{1a}, I wrote a Mathematica notebook of my own, taking some inspiration from Ref.~\cite{Hartle} (and I later loaned this notebook to Morgan Lynn):
