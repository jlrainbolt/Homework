...
\begin{solution}
	...
	\beqn \label{radpow}
		...
		= \frac{2 q^2 \alp^2 \omg^4}{3 c^3} \cos[2](\omg t - \frac{\omg}{c} r).
	\eeqn
	\vfix
\end{solution}
...
\begin{solution}
	...
	\beq
		\ev{\sEosc} = \frac{m \alp^2 \omg^2}{2} = \frac{k \alp^2}{2},
	\eeq
	and the average power radiated over one oscillation comes directly from \refeq{radpow} and $\ev{\cos^2x} = 1/2$,
	\beq
		\ev{P} = \frac{q^2 \alp^2 \omg^4}{3 c^3}.
	\eeq
	We can use these to write $Q$ with no $\alp$ dependence:
	\beq
		Q = \omg \frac{\ev{\sEosc}}{\ev{P}}
		= \omg \frac{m \alp^2 \omg^2}{2} \frac{3 c^3}{q^2 \alp^2 \omg^4}
