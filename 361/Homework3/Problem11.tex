...
\sol{
	...
	which is nonzero only for electrons of the same spin.  This means we can write an expression like Eq.~\refeq{thing11} for electrons of each spin state:
	\aln{ \label{energy11}
	...
	The total energy is the sum of the energies for each spin state: $E = \Eup + \Edn$.  The total electron density is the sum of the densities of spin up and of spin down electrons:
	\eqn{n11}{
		n = \frac{N}{V}
	...
	For a fully spin polarized state, either $\Nup$ or $\Ndn$ is 0 and the other $N$; that is, all electrons are in the same spin state.  Say $\Ndn = N$ and $\Nup = 0$.  Then $E = \Edn$, and
	\eq{
		\frac{\kdn^3}{6 \pi^2} = \frac{\kF}{3 \pi^2}
		\qimplies
		\kdn = 2^{1/3} \kF.
	}
	Making these substitutions in Eq.~\refeq{energy11}, we have~\cite[pp.~682--683]{Ashcroft}
	\aln{
