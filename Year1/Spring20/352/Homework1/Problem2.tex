...
\sol{
	...
	The state has zero total magnetization if $\Np = \Nm = N / 2$.  This is the same as Prob.~{1.1} with $\alp = t = 1/2$.  Feeding these into Eq.~\refeq{approx}, we find in the large $N$ limit
	\eq{
...
\sol{
	...
	\eq{
		...
		= \ans{ \sqrt{\frac{N}{2\pi \Np \Nm}} \frac{N^N}{\Np^{\Np} \Nm^{\Nm}}, }
...
\prob{Three-level system}{ \label{2.3}
...
\sol{
	...
	\aln{
		...
		&= N \ln N - \Np \ln \Np - \Nm \ln \Nm - \No \ln \No. \label{S2.3}
	}
	...
	so
	\eq{
		\eps = T \ln(\frac{N - \No - M}{N - \No + M})
		\qimplies
		E = M T \ln(\frac{N - \No - M}{N - \No + M})
	}
	is the relationship between temperature and total energy.  However, this expression includes the unconstrained variable $\No$.  If we assume the system is at equilibrium, in principle we may solve for $\No$ by maximizing entropy; that is, by fixing $\pdv*{S}{\No} = 0$.  However, this is a problem more easily solved used a canonical ensemble.  In the thermodynamic~($N \to \infty$) limit, the energy range for the canonical distribution becomes very narrow, and so it is practically equivalent to the microcanonical ensemble~\cite[pp.~78--79]{Landau}.

	For the canonical ensemble, we will assume the particles are indistinguishable and use the single-atom partition function $\Zii = \sum_n \exp(\eps_n / T)$~\cite[p.~87]{Landau}.  This is admissible even for a classical system because the energy levels in this problem are discrete.  For one particle in the three-level system, the partition function is
	\eq{
	...
	\eq{
		Z = \frac{1}{N!} \prodiN \Zii = \frac{1}{N!} \brac{ 1 + 2 \cosh(\frac{\eps}{T}) }^N.
	}
	...
	\al{
		S &= \ln Z + T \pdv{T} (\ln Z)
		\approx N \ln(1 + 2 \cosh(\frac{\eps}{T})) - N \ln N + N + N T \pdv{T} \left[ \ln(1 + 2 \cosh(\frac{\eps}{T})) - \ln N + 1 \right] \\
		&= N \ln(\frac{1}{N} + \frac{2}{N} \cosh(\frac{\eps}{T})) + N - N T \frac{\eps}{T^2} \frac{2 \sinh(\eps / T)}{1 + 2 \cosh(\eps / T)} \\
		&= \ans{ N \brac{ \ln(\frac{1}{N} + \frac{2}{N} \cosh(\frac{\eps}{T})) - \frac{2 \eps}{T} \frac{\sinh(\eps / T)}{1 + 2 \cosh(\eps / T)} + 1 }. }
	}
	...
	\eq{
		E \approx -N \pdv{}{\bet} \brac{ \ln(1 + 2 \cosh(\bet \eps)) - \ln N + 1 }
	...
	We note also that
	\eq{
		S = N \ln(\frac{1}{N} + \frac{2}{N} \cosh(\frac{\eps}{T})) + \frac{E}{T} + N.
	}
	
	Let $u = \eps / T$.  Then
	\al{
		S(u) &= N \brac{ \ln(1 + 2 \cosh u) - 2 u \frac{\sinh u}{1 + 2 \cosh u} - \ln N + 1 }, &
