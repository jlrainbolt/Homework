...
%		= \int \del\phisi \paren{ \pdv{\cL}{\phisi} - \partsm \pdv{\cL}{(\partsm\phisi)} } \dqx.
	...
	To vary the action, we stipulate that $\phisi$ is a solution of the Euler-Lagrange equations; that is, it extremizes the action for an \emph{arbitrary} variation.  This means $\delS = 0$.
	
	Then, substituting $\del\phisi = \am \partsm \phisi$,
	\aln{
	...
	is the total derivative of the Lagrangian~\cite[p.~82]{Landau}.  Substituting into Eq.~\refeq{delS2}, we have
	\eqn{delS2.2}{
	...
	where in going to the second equality we have inserted a factor of $\delmsn$~\cite[p.~83]{Landau}.  According to Jackson~(11.71), $\eta_{\mu \alp} \eta^{\alp \nu} = \delmsn$.  In the final equality, we have multiplied by $-1$ since $\delS = 0$.
...
\sol{
	...
	For a single continuous symmetry $\tht$ as we discussed in lecture, we found a conserved four-vector current $\Jm$.  For this problem, instead of writing $\am$ as a vector, we could have considered it as four single continuous symmetries: $a^0$, $a^1$, $a^2$, and $a^3$.  After varying the action four times, we would have found four conserved four-vector currents: $J^{\mu 0}$, $J^{\mu 1}$, $J^{\mu 2}$, and $J^{\mu 3}$.  Together, these currents are specified by sixteen elements.  A more compact way of writing these is as a two-index tensor $\Tmn$, which also has sixteen elements.
}