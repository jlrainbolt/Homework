...
\sol{
	...
	\eq{
		\chidia = -\frac{m^2 c \muB^2 T}{3\pi^2 \hbar^4} \intoi \frac{\ddpz}{\pz (1 + e^{\pz / 2m T - \mu / T })}.
	}
...
%	}
	...
	Note that
	\eq{
		\intoi \frac{k^s}{e^{k - \mu} + 1} \dd{k} = -\Gam(s + 1) \,\Li_{1 + s}(-e^{\mu}),
	}
	where $\Li$ is the polylogarithm~\cite{Polylog}.  So we have
	\eq{
		\chidia = -\frac{m^2 c \muB^2 T}{3\pi^2 \hbar^4} \intoi \frac{z^{-1}}{1 + e^{z - \mu / T }} \ddz
		= \frac{m^2 c \muB^2 T}{3\pi^2 \hbar^4} \Gam(0) \,\Li_0(-e^\mu),
	}
	where $\Gam(0)$ diverges.
...
\sol{
	...
	For $T \ll \EF = \po^2 / 2m$ where $\po$ is the Fermi momentum, the integral has contributions from only $p \in (0, \po)$.  Thus
	\eq{
		I = \intoi \frac{p\, \Jo(p r / \hbar)}{e^{(p^2 - \po^2) / 2m T} + 1} \ddp
		\approx \intoi \frac{p\, \Jo(p r / \hbar)}{e^{(p^2 - \po^2) / 2m T}} \ddp
		= m T \exp(\frac{\po^2}{2 m T} - \frac{m T r^2}{2 \hbar^2}),
	}
	where we have evaluated the integral using Mathematica.  Substituting into Eq.~\refeq{thing5},
	\eq{
		\nu(r) = \frac{g m^2 T^2}{4 \pi^2 \hbar^4 \evn} \exp(\frac{\po^2}{m T} - \frac{m T r^2}{\hbar^2}),
	}
	and feeding this into Eq.~\refeq{C}, we find
	\eq{
		\ans{ C(r) = \evn \del(r) + \frac{g m^2 T^2}{4 \pi^2 \hbar^4} \exp(\frac{\po^2}{m T} - \frac{m T r^2}{\hbar^2}). }
	}
...
%		&= \frac{\hbar \po^2}{2 m^2 T^2} \paren{ \frac{\po^2}{\hbar} \int_a^\infty \frac{\ddx}{(e^x + 1) (e^{-x} + 1)} + 2 \lam \po \intoi \frac{x}{(e^x + 1) (e^{-x} + 1)} \ddp + \hbar \lam^2 \intoi \frac{x^2}{(e^x + 1) (e^{-x} + 1)} \ddp } 
