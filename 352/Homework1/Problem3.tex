...
\sol{
	...
	All three subsystems have per-particle energies that are not quantized.  Assuming the particles are indistinguishable, the general expression for the partition function in this case is~\cite[pp.~55--56]{Pathria}
	\eq{
...
%		= \frac{\pi}{2} \frac{2 m L^2 T}{(2\pi \hbar)^2} \intoi u^2 e^{-u} \frac{L \sqrt{2 m T}}{2\pi\hbar} \dd{u} 
	...
	\al{
		\limTo \ZK &\approx \frac{1}{N!} \brac{ 1 + 6 \exp(-\frac{(2\pi\hbar)^2}{2 m V^{3/2} T}) }^N, &
		\limTo \ZV &\approx \frac{1}{N!} \paren{ e^{-\hbar \omg / 2 T} }^N, &
		\limTo \ZR &\approx \frac{1}{N!} \paren{ 1 + 3 e^{-\hbar^2 / I T} }^N,
	}
	whose energies we can find using Eq.~\refeq{EfromZ}.  We find~\cite[p.~161]{Kardar}
	\aln{
		\limTo \EK &= -\pdv{\bet} (\ln \ZK)
		\approx -\pdv{\bet} \brac{ N \ln(1 + 6 \exp(-\frac{(2\pi\hbar)^2 \bet}{2 m V^{3/2}})) - \ln N! }
		...
		\approx -\pdv{\bet} \paren{ -N \frac{\hbar \omg \bet}{2} - \ln N! }
		= N \frac{\hbar \omg}{2} \label{EV3.2}, \\[2ex]
		\limTo \ER &= -\pdv{\bet} (\ln \ZR)
		\approx -\pdv{\bet} \brac{ N \ln(1 + 3 e^{-\hbar^2 \bet / I}) - \ln N! }
...
\sol{
	...
	\aln{
		...
		\approx \ln\ZK + T \pdv{T} \brac{ N \ln(1 + 6 e^{-(2\pi\hbar)^2 / 2 m V^{3/2} T}) - \ln N! } \notag \\
		&\approx N \ln(1 + 6 \exp(-\frac{(2\pi\hbar)^2}{2 m V^{3/2} T}) ) - N \ln N + N + \frac{3 N}{T} \frac{(2\pi\hbar)^2}{m V^{3/2}} \exp(-\frac{(2\pi\hbar)^2}{2 m V^{3/2} T}) \notag \\
		&= \ans{ N \ln(\frac{1}{N} + 2 \frac{m V^{3/2} \EK}{N^2 (2\pi\hbar)^2}) + 2 \frac{m V^{3/2} \EK}{(2\pi\hbar)^2} \ln(\frac{3 N (2\pi\hbar)^2}{m V^{3/2} \EK}) + N, } \label{SK3.2} \\[2ex]
		\limTo \SV &= \pdv{T}(T \ln \ZV) = \ln \ZV + T \pdv{T}(\ln \ZV)
		= -N \frac{\hbar \omg}{2 T} - \ln N! + T \pdv{T} \paren{ -N \frac{\hbar \omg}{2 T} - \ln N! } \notag \\
		&= -N \frac{\hbar \omg}{2 T} - N \ln N + N + \frac{N}{2} \frac{\hbar \omg}{T}
		= \ans{ N - N \ln N, } \label{SV3.2} \\[2ex]
		\limTo \SR &= \pdv{T}(T \ln \ZR) = \ln \ZR + T \pdv{T}(\ln \ZR)
		= N \ln(1 + 3 e^{-\hbar^2 / I T}) - \ln N! + T \pdv{T} \brac{ N \ln(1 + 3 e^{-\hbar^2 / I T}) } \notag \\
		&\approx N \ln(1 + 3 e^{-\hbar^2 / I T}) - \ln N! + \frac{3 N}{T} \frac{\hbar^2}{I} e^{-\hbar^2 / I T}
		= \ans{ N \ln(\frac{1}{N} + \frac{I \ER}{N^2 \hbar^2}) + \frac{I \ER}{\hbar^2} \ln(\frac{3 N \hbar^2}{I \ER}) + N, } \label{SR3.2}
	}
...
\sol{
	...
	For the low temperature limit, we need to solve Eqs.~(\ref{SK3.2}--\ref{SR3.2}) for $\EK$, $\EV$, and $\ER$.  For Eqs.~(\ref{SK3.2}, \ref{SR3.2}), this is only possible if we eliminate some of the terms in the $T \to 0$ limit.  The $(\alp / T) e^{-\alp / T}$ terms tend to 0 more quickly than the $\ln(1 + e^{-\alp / T})$ terms.  However, even if we kept only these terms, we would have an expression of the form
	\eq{
		S = \frac{E}{\alp} \ln(\frac{\alp}{E}),
	}
	which does not have a single-valued solution for $E$.

	For $\EV$, however, the energy is not a function of entropy, it is simply
	\eq{
		\ans{ \limTo \EV = N \frac{\hbar\omg}{2}, }
	}
	as we found in Eq.~\refeq{EV3.2}.
}
...
\sol{
	...
	\al{
		\ans{ \limTo \SK\ } &\ans{\approx N \ln(\frac{1}{N} + \frac{6}{N} \exp(-\frac{(2\pi\hbar)^2}{2 m V^{3/2} T}) ) + \frac{3 N}{T} \frac{(2\pi\hbar)^2}{m V^{3/2}} \exp(-\frac{(2\pi\hbar)^2}{2 m V^{3/2} T}) + N, } \\[1.5ex]
		\ans{ \limTo \SV\ } &\ans{\approx N - N \ln N, } \\[1.5ex]
		\ans{ \limTo \SR\ } &\ans{\approx N \ln(\frac{1}{N} + 3 e^{-\hbar^2 / I T}) + \frac{3 N}{T} \frac{\hbar^2}{I} e^{-\hbar^2 \bet / I} + N. }
	}
	\vfix
}
...
\sol{
	...
	\al{
		\limTo \FK &= -T \ln \ZK
		\approx \ans{ -N T \brac{ \ln(\frac{1}{N} + \frac{6}{N} \exp(-\frac{(2\pi\hbar)^2}{2 m V^{3/2} T}) ) + 1 }, } \\[1.5ex]
		\limTo \FV &= -T \ln \ZV
		\approx \ans{ N \paren{ \frac{\hbar \omg}{2} - \ln N + 1}, } \\[1.5ex]
		\limTo \FR &= -T \ln \ZR
		\approx \ans{ -N T \brac{ \ln(\frac{1}{N} + \frac{3}{N} e^{-\hbar^2 / I T}) + 1 } }.
	}
	\vfix
}