...
\sol{
	...
	To determine whether there are higher multipoles, we will examine Eq.~\refeq{qhlm}.  We know from the fact that $\rhon(\vx, t)$ exists for all $n = m \geq 0$, so multipoles for a given $m$ would not exist only if $\Plm(\cos\tht) = 0$ for all $l \geq m$.  However, the associated Legendre polynomials $\Plm$ are given by~\cite{ALP}
	\eq{
