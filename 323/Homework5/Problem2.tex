...
\sol{
	...
	Note that each charge is a distance $a / \sqrt{2}$ from the origin.  The locations of each of the point charges are
	\al{
	...
	\al{
		...
		= -3i q a^2 \sin\tht \,e^{-2i \omg t} \,e^{i \phi}, \\
		\Qz &= 0.
	}
	Then
	\al{
	...
	The electric field for quadrupole radiation is given by Jackson~(9.132),
	\eq{
		\vE_{l m} = \Zo \vH_{l m} \cross \nh,
	}
	since pure quadrupole radiation is equivalent to a linear combination of $q_{2 m}$ multipoles as stated on Jackson p.~437.  One of the vector identities on the inside cover is
	\eq{
	...
	and
	\eq{
		\vE = -\Zo \frac{i c k^3}{24\pi} \frac{e^{i k r}}{r} \{ \vQ(\nh) - [ \nh \vdot \vQ(\nh) ] \nh \}.
	}
	Note that
	\eq{
...
%		&= 3q a^2 \sin\tht \,e^{-2i \omg t} \,e^{i\phi} \brac{ \paren{ \frac{1 - \cos(2\tht)}{2} \cos\phi \,e^{i \phi} - 1 } \,\xh + \paren{ \frac{1 - \cos(2\tht)}{2} \sin\phi \,e^{i \phi} - i } \,\yh + \frac{\sin(2\tht)}{2} \,e^{i\phi} \,\zh }
	}
	Then
	\eq{
	...
	Since $\Qij \propto e^{2i \omg t}$, \ans{the frequency of radiation is $2\omg$.}  This means $k = 2 \omg / c$.  Writing the previous results in terms of the frequency, we have
	\ans{ \al{
