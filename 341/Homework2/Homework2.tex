...
\begin{statement}
	...
	\begin{align}
		X(phi) &= \Ud(\phi) X U(\phi), \label{defx} \\
		Y(\phi) &= \Ud(\phi) Y U(\phi). \label{defy}
	\end{align}
	Note that $X(0) = Y(0) = 0$.
\end{statement}
...
\begin{problem}
	Derive the equation
	\begin{equation}
		\dv{X(\phi)}{\phi} = \frac{i}{\hbar} \Ud(\phi) [\Le, X] U(\phi) = -Y(\phi),
	\end{equation}
	and a similar equation for $\dv*{Y(\phi)}{\phi}$.
\end{problem}

\begin{solution}
	Using the definition of $X = X(\phi)$ in \refeq{defx} and applying the product rule of differentiation,
	\begin{align}
		\dv{X}{\phi} &= \dv{}{\phi} (\Ud X U) = \dv{\Ud}{\phi} X U + \Ud \dv{}{\phi} (X U) = \dv{\Ud}{\phi} X U + \Ud \dv{X}{\phi} U + \Ud X \dv{U}{\phi}
	\end{align}
\end{solution}
