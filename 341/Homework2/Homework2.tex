...
\begin{solution}
	...
	so
	\begin{equation}
		\pdv{}{t} \evHot = -\frac{i}{\hbar} \ev{[\Ho, \Ht]}{\Psi(t)} = -\frac{i}{\hbar} \Ft \frac{\ihb}{m} \ev{P}{\Psi(t)} = \frac{\Ft}{m} \ev{P}.
	\end{equation}
\end{solution}
...
\begin{solution}
%	From the defining equation for Heisenberg operators,
%	\begin{align}
%		\Xt &= \Udt X \Ut, &
%		\Pt &= \Udt P \Ut
%	\end{align}
...
%	\begin{equation}
%		\Ut = \exp(-\frac{i}{\hbar} H t ) = \exp(-\frac{i P^2 t}{2\hbar m} - \frac{i m \omega^2 X^2 t}{2 \hbar}).
%	\end{equation}
...
\begin{solution}
	...
	\begin{align}
		...
		p(t) &= m \omega B \cos(\omega t) - m \omega A \sin(\omega t) \label{momsol}
	\end{align}
	where $A$ and $B$ are constants.  To find \refeq{momsol}, we have applied \refeq{poseq}.
...
\begin{problem}
	...
	\begin{equation}
		\braket{x}{\Psi(0)} = \left( \frac{m \omega}{\pi \hbar} \right)^{1/4} \exp(-\frac{m \omega (x - b)^2}{2 \hbar}).
	\end{equation}
