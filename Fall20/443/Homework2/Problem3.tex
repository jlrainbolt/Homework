...
\sol{
	...
	where we have transposed the first term.  This is allowed because it is a c-number, and the sum of the matrix elements is the same if the matrices are transposed.  We have also used the fact that $\sigm$ is Hermitian, since all $\sigi$ are~\cite[p.~165]{Sakurai}.  Then, integrating by parts, note that
	\eq{
...
\sol{
	...
	where we have applied Eq.~\refeq{sigthing} and transposed the second term.  Since $\cLDirac$ is a Lagrangian density, we may integrate this term by parts to obtain
	\eq{
