...
\sol{
	...
	Using this equation and the identity $\gamm \gamsm = 4$~\hl{[cite]}, Eq.~\refeq{thingy} can be written
	\eqn{Z1}{
	...
	\aln{
		...
		&\approx \frac{i}{32 \pi^2} \brac{ 2 \ln(\frac{\Del +\Lam^2}{\Del}) - 3 }
		\equiv \bet, \label{ans2}
	}
	where we have defined $\bet$ and ignored terms of $\order{\Lam^2}$.
	
	Setting $q^2 = 0$ (so $\Del \to \Delo = (1 - z)^2 m^2$, and $\alp \to \alpo, \bet \to \beto$ which are functions of $\Delo$) and feeding in Eqs.~\refeq{ans1} and \refeq{ans2}, Eq.~\refeq{thing1} can be written
	\eq{
	...
	Feeding these results into Eq.~\refeq{Z1}, we obtain
	\eq{
		\ans{ \del\Zq = -2 i e^2 \intoq \ddz (1 - z) \brac{ -\beto + 2 (1 - 4z + z^2) m^2 \alpo } + \gamsm \frac{e^2 \sigmn \qsn}{m} \intoq \ddz m^2 z (1 - z)^2 \alpo, }
	}
	where 
	\al{
		\alpo &= -\frac{i}{32 \pi^2} \frac{\Lam^4}{\Delo (\Delo + \Lam^2)^2}, &
		\beto &= \frac{i}{32 \pi^2} \brac{ 2 \ln(\frac{\Delo +\Lam^2}{\Delo}) - 3 },
	}
	and $\Delo = (1 - z)^2 m^2$.
	...
	where we have evaluated the integral using Mathematica and ignored terms of $\order{\Lam^2}$.  Substituting back into Eq.~\refeq{Sigw}, we find
	\eq{
	...
	\al{
		...
		\dv{\Del}\brac{ \ln(\frac{\Del + \Lam^2}{\Del}) } &= \dv{\Del}\brac{ \ln(\Del + \Lam^2) - \ln(\Del) }
		= \frac{1}{\Del + \Lam^2} - \frac{1}{\Del}.
	}
	Making these substitutions into Eq.~\refeq{der}
	\al{
		\dv{\Sigw}{\psl} &= \frac{e^2}{16 \pi^2} \intoq \ddx \curly{ -2 x \brac{ \ln(\frac{\Del + \Lam^2}{\Del}) - 1 } + (-2 x \psl + 4 \mo) [ -2x (1 - x) \psl ] \paren{ \frac{1}{\Del + \Lam^2} - \frac{1}{\Del} } - 1 } \\
		&= \frac{e^2}{16 \pi^2} \intoq \ddx \curly{ -2x  \brac{ \ln(\frac{\Del + \Lam^2}{\Del}) - 1 } + \frac{(2 x \psl + 4 \mo) [ 2x (1 - x) \psl ]}{\Del} - 1 },
	}
	again omitting terms of $\order{\Lam^2}$.  Then Eq.~\refeq{Z2} becomes, defining $\Delm \equiv -x (1 - x) m^2 + x \mu^2 + (1 - x) \mo^2$,
	\eq{
		\del\Zw = \frac{e^2}{16 \pi^2} \intoq \ddx \curly{ -2x  \brac{ \ln(\frac{\Delm + \Lam^2}{\Delm}) - 1 } + \frac{(2 x m + 4 \mo) [ 2x (1 - x) m ]}{\Delm} - 1 }.
	}
	
	\al{
		\del\Zq &= -2 i e^2 \intoq \ddz (1 - z) \curly{ -\frac{i}{32 \pi^2} \brac{ 2 \ln(\frac{\Delo +\Lam^2}{\Delo}) - 3 } + 2 (1 - 4z + z^2) m^2 \paren{ -\frac{i}{32 \pi^2} \frac{\Lam^4}{\Delo (\Delo + \Lam^2)^2} } } \\
		& \hspace{5em} \phantom{=\ } + \gamsm \frac{e^2 \sigmn \qsn}{m} \intoq \ddz m^2 z (1 - z)^2 \paren{ -\frac{i}{32 \pi^2} \frac{\Lam^4}{\Delo (\Delo + \Lam^2)^2} },
	}
	
	\hl{need to add mu to other Delta?  And can we just use the first term like they seem to on p. 222?}
}
