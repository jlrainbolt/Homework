...
\sol{
	...
	These results are consistent with the Mermin--Wagner theorem, which states that a continuous symmetry cannot be broken in dimensions $d \leq 2$~\cite{CMW}.  That is, in $d \leq 2$ dimensions, a symmetry-breaking field cannot have a nonzero vacuum expectation value~\cite[p.~460]{Peskin}.  This is because each spin has more nearest neighbors in higher dimensions; the spins are inclined to align with their neighbors, thus there is a higher degree of correlation in higher dimensions at the same distance.  In two dimensions, \hl{finish this}
}
