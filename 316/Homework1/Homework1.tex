...
%		provided $Q_j$ and $\Qd_j$ are continuously differentiable with respect to $q_1, q_2, \ldots, q_n$.
	...
	\item \begin{problem}
		...
		\begin{enumerate}
			...
			\item If $k = 2$, the forces correspond to a system of harmonic oscillators couples to each other.  Show that the result in part (a) implies the frequencies of such a system are independent of the oscillation amplitude.
	...
	\begin{solution}
		...
		\begin{enumerate}
			...
			\begin{equation}
				\alpha^k \beta^2 = 1 \implies \beta = \alpha^{-k/2}.
			\end{equation}
			(The solution where $\beta < 1$ is spurious unless we are interested in running time backwards.)
