...
\sol{
	...
	\eqn{lagr5}{
		S[\vx] = \int \paren{ -m c^2 \sqrt{1 - \frac{\vxd^2}{c^2}} - q \phi + \frac{q}{c} \vxd \vdot \vA } \ddt
		\equiv \int \cL(\vx, \vxd) \ddt,
	}
	...
	\eq{
		\pdv{\cL}{\xdii} = -mc^2 \paren{ \frac{1}{2} \frac{1}{\sqrt{ 1 - \vxd^2 / c^2}} } \paren{ -\frac{2 \xdii}{c^2} } + \frac{q}{c} \Aii
		= \frac{m \xdii}{\sqrt{1 - \vxd^2 / c^2}} + \frac{q}{c} \Aii
		= \pii + \frac{q}{c} \Aii,
	}
	since $\vp = m \gam \vxd$ and $\bet = \vxd / c$.
	
	Secondly, we have~\cite[p.~50]{Landau}
	\eq{
		\pdv{\cL}{\vx} = \grad \cL
		= \frac{q}{c} \grad(\vxd \vdot \vA) - q \grad\phi.
	}
	...
	so~\cite[p.~50]{Landau}
	\eq{
		\grad(\vxd \vdot \vA) = (\vxd \vdot \grad) \vA + \vxd \cross (\grad \cross \vA)
		\qimplies
		\pdv{\cL}{\vx} = \frac{q}{c} [ (\vxd \vdot \grad) \vA + \vxd \cross (\grad \cross \vA) ] - q \grad\phi.
	}
	
	Then the Euler-Lagrange equations become
	\eqn{el5}{
		\dv{\vp}{t} + \frac{q}{c} \dv{\vA}{t} = \frac{q}{c} [ (\vxd \vdot \grad) \vA + \vxd \cross (\grad \cross \vA) ] - q \grad\phi.
	}
	The total derivative of $\vA$ is given by~\cite[p.~50]{Landau},
	\eq{
		\dv{\vA}{t} = \pdv{\vA}{t} + \vxd \vdot \pdv{\vA}{\vxd}
		= \pdv{\vA}{t} + (\vxd \vdot \grad) \vA.
	}
	Then Eq.~\refeq{el5} becomes
	\eqn{el5.2}{
		\dv{\vp}{t} = \frac{q}{c} \vxd \cross (\grad \cross \vA) - q \grad\phi - \frac{q}{c} \dv{\vA}{t}.
	}
	...
	Making these substitutions and $\vxd \to \vv$ in Eq.~\refeq{el5.2}, we have
	\eqn{Lorentz}{
...
\sol{
	...
	From Wald~(5.17), $\dv*{\cE}{t} = \vJ \vdot \vE$.  For a point charge, $\vJ = q \vv$.  Making this substitution, dividing by $m c$, and changing to a derivative of $\tau$, we find~\cite[p.~553]{Jackson}
	\eqn{temporal}{
	...
	\eq{
		\ans{ \dv{\Um}{\tau} = \frac{q}{m c} \Fmn \Usn }
	}
	as desired. \qed
}